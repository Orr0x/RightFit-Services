...
interface WorkerReportsTabProps {
  refreshKey: number
  onRefresh: () => void
}

export default function WorkerReportsTab({ refreshKey, onRefresh }: WorkerReportsTabProps) {
  ...
  const loadIssues = () => {
    withLoading(async () => {
      try {
        ...
        const response = await fetch(`/api/customer-portal/worker-issues?customer_id=${customer.id}`)

        if (!response.ok) throw new Error('Failed to load worker issues')

        const data = await response.json()
        // Only show SUBMITTED or CUSTOMER_REVIEWING issues
        const pending = data.data.filter((issue: WorkerIssue) =>
  ...
  const handleApprove = async (issueId: string) => {
    try {
      ...
      const response = await fetch(`/api/customer-portal/worker-issues/${issueId}/approve`, {
        method: 'POST',
  ...
  const handleReject = async (issueId: string) => {
    try {
      ...
      const response = await fetch(`/api/customer-portal/worker-issues/${issueId}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  if (issues.length === 0) {
    return (
      <EmptyState
        ...
        description="Worker issues awaiting your approval will appear here"
      />
    )
  }
