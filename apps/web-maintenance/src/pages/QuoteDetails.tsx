...
import { maintenanceQuotesAPI, type MaintenanceQuote } from '../lib/api'
...
export default function QuoteDetails() {
  ...
  const [quote, setQuote] = useState<MaintenanceQuote | null>(null)
  ...
  const loadQuote = async () => {
    try {
      ...
      const data = await maintenanceQuotesAPI.get(id!)
      setQuote(data)
    } catch (err: any) {
  ...
  const handleApprove = async () => {
    await withLoading(async () => {
      try {
        await maintenanceQuotesAPI.approve(id!)
  ...
  const handleDecline = async () => {
    await withLoading(async () => {
      try {
        await maintenanceQuotesAPI.decline(id!, declineReason)
  ...
  const handleSend = async () => {
    await withLoading(async () => {
      try {
        await maintenanceQuotesAPI.send(id!)
  ...
  const handleDelete = async () => {
    ...
    await withLoading(async () => {
      try {
        await maintenanceQuotesAPI.delete(id!)
