...
export default function MaintenanceDashboard() {
  ...
  const handleJobClick = (job: MaintenanceJob) => {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {activeJobs.length === 0 ? (
        ...
        <>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600 space-y-1">
                          ...
                          {job.quote && (
                            <div>
                              <strong>Quote:</strong> {job.quote.quote_number} - £{Number(job.quote.total).toFixed(2)} ({job.quote.status})
                            </div>
                          )}
                        </div>
                      ...
                      <div className="text-right">
                        ...
                        {job.actual_total && (
                          <div className="text-sm text-green-600 font-semibold">
                            Actual: £{Number(job.actual_total).toFixed(2)}
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {viewMode === 'calendar' && (
            <Card className="p-6">
              <CalendarView jobs={activeJobs} onJobClick={handleJobClick} />
            </Card>
          )}
          ...
          {viewMode === 'kanban' && (
            <Card className="p-6">
              <KanbanView jobs={activeJobs} onJobClick={handleJobClick} />
            </Card>
          )}
        </>
      )}
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="p-4">
          ...
          <div className="text-sm text-gray-600">
            <div>Estimated: £{activeJobs.reduce((sum, j) => sum + Number(j.estimated_total || 0), 0).toFixed(2)}</div>
            <div>Actual: £{activeJobs.reduce((sum, j) => sum + Number(j.actual_total || 0), 0).toFixed(2)}</div>
          </div>
        </Card>
      </div>
    </div>
