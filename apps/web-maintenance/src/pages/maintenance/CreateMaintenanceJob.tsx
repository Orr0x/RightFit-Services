...
export default function CreateMaintenanceJob() {
  ...
  const loadJob = async () => {
    ...
    try {
      const job = await maintenanceJobsAPI.get(id!)
      setFormData({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    withLoading(async () => {
      try {
        const submitData: CreateMaintenanceJobData = {
          ...formData,
        ...
        if (isEditMode && id) {
          await maintenanceJobsAPI.update(id, submitData)
          const successMsg = scheduleNow && formData.status === 'PENDING'
            ? 'Job scheduled successfully'
            : 'Cleaning job updated successfully'
          toast.success(successMsg)
        } else {
          const job = await maintenanceJobsAPI.create(submitData)
    ...
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-bold">
          {isEditMode ? 'Edit Maintenance Job' : 'Create Maintenance Job'}
        </h1>
      </div>
