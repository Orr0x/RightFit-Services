...
class MaintenanceContractService {
  ...
  async linkProperty(contract_id: string, data: LinkPropertyDTO) {
    ...
    const existing = await prisma.maintenanceContractProperty.findFirst({
      where: {
        contract_id,
    ...
    const contractProperty = await prisma.maintenanceContractProperty.create({
      data: {
        contract_id,
        property_id: data.property_id,
  ...
  async unlinkProperty(contract_id: string, property_id: string) {
    const contractProperty = await prisma.maintenanceContractProperty.findFirst({
      where: {
        contract_id,
        property_id,
        is_active: true,
    ...
    const updated = await prisma.maintenanceContractProperty.update({
  ...
  async getContractProperties(contract_id: string) {
    const properties = await prisma.maintenanceContractProperty.findMany({
      where: {
        contract_id,
        is_active: true,
      },
  ...
  async calculateMonthlyFee(contract_id: string): Promise<number> {
    ...
    const total = contract.maintenance_property_links.reduce((sum, cp) => {
...
export default new MaintenanceContractService()
