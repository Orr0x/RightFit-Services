...
export class MaintenanceQuoteService {
  /**
  ...
  async list(
    ...
    const [quotes, total] = await Promise.all([
      prisma.maintenanceQuote.findMany({
        where,
        skip,
        take: limit,
        orderBy: [
      ...
      prisma.maintenanceQuote.count({ where }),
    ]);
  ...
  async getById(id: string, serviceProviderId: string) {
    const quote = await prisma.maintenanceQuote.findFirst({
      where: {
        id,
        customer: {
          service_provider_id: serviceProviderId,
  ...
  async update(
    ...
    const quote = await prisma.maintenanceQuote.update({
      where: { id },
      data: updateData,
      include: {
        customer: true,
  ...
  async delete(id: string, serviceProviderId: string) {
    ...
    await prisma.maintenanceQuote.delete({
      where: { id },
    });
  ...
  async generateQuoteNumber(): Promise<string> {
    ...
    const count = await prisma.maintenanceQuote.count({
      where: {
        created_at: {
          gte: startOfMonth,
          lte: endOfMonth,
  ...
  async getCustomerStats(customerId: string, serviceProviderId: string) {
    const stats = await prisma.maintenanceQuote.aggregate({
      where: {
    ...
    const approvedCount = await prisma.maintenanceQuote.count({
      where: {
        customer_id: customerId,
        status: 'APPROVED',
        customer: {
    ...
    const pendingCount = await prisma.maintenanceQuote.count({
      where: {
        customer_id: customerId,
        status: 'SENT',
        customer: {
    ...
    const declinedCount = await prisma.maintenanceQuote.count({
      where: {
        customer_id: customerId,
        status: 'DECLINED',
        customer: {
