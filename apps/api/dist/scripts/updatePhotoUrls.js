...
async function updatePhotoUrls() {
    ...
    for (const photo of photos) {
        ...
        if (photo.s3_url.includes('localhost')) {
            updates.s3_url = photo.s3_url.replace(/http:\/\/localhost:\d+/, apiBaseUrl);
        }
        if (photo.thumbnail_url?.includes('localhost')) {
            updates.thumbnail_url = photo.thumbnail_url.replace(/http:\/\/localhost:\d+/, apiBaseUrl);
        }
...
//# sourceMappingURL=updatePhotoUrls.js.map