...
class EmailService {
    ...
    constructor() {
        ...
        if (apiKey) {
            ...
            logger_1.default.info('Resend email service configured');
        }
        else {
            logger_1.default.warn('Resend API key not configured - email notifications will be logged only');
        }
    }
    /**
...
RightFit Services
      `;
            if (this.isConfigured && this.resend) {
                ...
                logger_1.default.info('Certificate expiry email sent', {
                    to,
                ...
                logger_1.default.info('Would send certificate expiry email (Resend not configured)', {
                    to,
                    subject,
                    certificate_type: certificateType,
                    days_until_expiry: daysUntilExpiry,
            ...
            logger_1.default.error('Failed to send certificate expiry email', {
                error: error.message,
                to: params.to,
            });
            throw error;
    ...
    async sendWelcomeEmail(params) {
        try {
          ...
          `,
                ...
                logger_1.default.info('Welcome email sent', { to });
            }
            else {
                logger_1.default.info('Would send welcome email (Resend not configured)', { to });
            }
        }
        catch (error) {
...
exports.default = new EmailService(); // Default export for production
