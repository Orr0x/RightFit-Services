...
class NotificationService {
     ...
     */
    async checkAndSendCertificateExpiryNotifications() {
        try {
            ...
            for (const days of alertDays) {
                ...
                for (const cert of expiringCertificates) {
                    ...
                    // Log notification
                    logger_1.default.info('Certificate expiry notification', {
                ...
                if (expiringCertificates.length > 0) {
                    logger_1.default.info(`Sent ${expiringCertificates.length} certificate expiry notifications for ${days} days`, {
            ...
            logger_1.default.error('Error checking certificate expiry notifications', {
                error: error.message,
            });
        }
    }
    ...
    async checkExpiredCertificates() {
        ...
        for (const cert of expiredCertificates) {
            ...
            logger_1.default.warn('Expired certificate detected', {
                certificate_id: cert.id,
     ...
     */
    async sendPushNotification(notification, daysUntilExpiry) {
        try {
            ...
            if (!certificate || !certificate.property) {
                logger_1.default.warn('Certificate or property not found', {
                    certificate_id: notification.certificateId,
                });
                return;
            }
            // Send push notification
            await PushNotificationService_1.default.sendNotification({
            ...
            logger_1.default.info('Push notification sent for certificate expiry', {
                certificate_id: notification.certificateId,
                days_until_expiry: daysUntilExpiry,
            });
        }
        catch (error) {
            logger_1.default.error('Failed to send push notification for certificate expiry', {
                error: error.message,
                certificate_id: notification.certificateId,
            });
        }
    ...
    async sendEmailNotification(notification) {
        try {
            await EmailService_1.default.sendCertificateExpiryEmail({
     ...
     */
    async sendUrgentExpiredNotification(notification) {
        logger_1.default.error('URGENT: Certificate has expired', {
            certificate_id: notification.certificateId,
            property: notification.propertyName,
            type: notification.certificateType,
     ...
     */
    async getCertificateSummary(tenantId) {
        const now = new Date();
